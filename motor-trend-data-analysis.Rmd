...
single regressor (transmission type).

```{r linear-models, echo = TRUE}
singleModelFit <- lm(mpg ~ factor(am), data = mtcars)
...
is around 0.338. That means that this first model can only explain 33.8% of the data. So there may be other variables which could give a better understanding of the outcome. We know that adding more variables increases the R2 coefficient. So we expect a higher value of this coefficient by adding more predictors into the model.
...
1. **Is an automatic or manual transmission better for MPG?**
    ...
    vehicles with an automatic transmission. However, 
    
    ... when modeled with confounding variables like displacement, HP and
    weight, the difference is not as significant as it seems in the beginning:
    a big part of the difference is explained by other variables.
    
    ... other variables such as
    the number of cylinders, displacement, gross horsepower and weight are more
    statistically significant when determining fuel consumption.

1. **Quantify the MPG difference between automatic and manual transmissions.**

    An automobile with a manual transmission
    will achieve `r round(t1$estimate[1], 2)` MPG on average compared to
    `r round(t1$estimate[2], 2)` MPG for an automatic
    transmission giving us a mean difference of
    `r round(t1$estimate[2] - t1$estimate[1], 2)` MPG favoring a manual
    transmission.
...
---
the MPG difference between manual and automatic transmissions is

7.245 MPG - for the simple linear regression model (mpg ~ am)

2.936 MPG - for the best fitted multiple regression model (mpg ~ wt + qsec + am)
